...
void GMBrowser::DrawPage(int page)
{
  ...
  if (TotalEvents > 0) {
    char line[80];
    snprintf(line, 80, "  Run: %d    SubRun: %d    Total Events: %d", fRun, fSubRun, TotalEvents);
  ...
  for (unsigned int hist = 0; hist < thePage->Name.size(); hist++)
	...
	Info("DrawPage","\tDraw: %s", name.c_str());
      ...
      //if (fRunNumber > 0 && !thePage->ShowRunNumber)
        // GAF - 24 Aug 2010 - separate run and subrun number 
        //snprintf(title, "Run: %6.6d - %s", fRunNumber, hst->GetTitle());
        //snprintf(title, "Run: %4.4d SubRun: %4.4d - %s", fRun, fSubRun, hst->GetTitle());
      //else
        snprintf(title, 132, "%s", hst->GetTitle());
	...
	// Do the same for the 2nd reference histogram
	  ...
	  char title[132];
	  snprintf(title, 132, "%s (Prob: %1.3f) ", hst->GetTitle(), prob);
	  hst->SetTitle(&title[0]);
	}
	...
	gStyle->SetOptStat(option_stat_value);
      ...
      if (thePage->ShowRunNumber && hist == 0) {
        ...
        // GAF - 24 Aug 2010 - separate run and subrun number 
        ///snprintf(run_label,"Run: %6.6d", fRunNumber); // make label
        snprintf(run_label, 150, "Run: %4.4d SubRun: %4.4d",fRun, fSubRun); // make label
  ...
  switch (id) 
	...
	// Open file with plot description. Expand any environment variables.
	  ...
	  new ifstream(gSystem->ExpandPathName(thePage->HelpFile.c_str()));
        ...
        // Title line
        char line[fLineSize];
        snprintf(line, fLineSize, "Information for page: %s", thePage->PageName.c_str());
  	hd->SetText(line);

	// Make sure file is open
        if ( !input || (input && (input->fail() || input->bad()) ) ) {
          snprintf(line, fLineSize, "No plot information available.");
          hd->AddText(line);
	}
        else {
...
void GMBrowser::saveAs(const char *type, const char *name)
{
  ...
  if (filename.Contains(".ps")  ||
    ...
    Canvas()->SaveAs(filename.Data());
      ...
      gVirtualX->Instance()->WriteGIF((char *)filename.Data());
    }
  else
    Warning("handleMenu", 
...
void GMBrowser::savePS(int ipage)
{
  ...
  if (ipage == 0) {
    ...
    if (file_open) {
      ...
      // GAF - 24 Aug 2010 - separate run and subrun number 
      /*snprintf(command_string,"mv %s %s/%s_run%6.6d.ps",
        tmp_file.c_str(),
        fFileSaveInfo.fIniDir, 
        fTitle.c_str(),
        fRunNumber);*/
      snprintf(command_string,256, "mv %s %s/%s_run%4.4d_%4.4d.ps",
        tmp_file.c_str(),
        fFileSaveInfo.fIniDir, 
        fTitle.c_str(),
        fRun,
     ...
     //cout << "Executing command: " << command_string << endl;
      ...
      // Compress the file
      snprintf(command_string,256, "gzip -v --force %s/%s_run%4.4d_%4.4d.ps",
        fFileSaveInfo.fIniDir, 
        fTitle.c_str(),
        fRun,
        fSubRun);
...
void GMBrowser::print(Bool_t callDialog)
{
  ...
  char filename[160];
  snprintf(filename,160,"%s/GMpage%2.2d.ps", fFileSaveInfo.fIniDir, 
          fCurrentPage);
...
string GMBrowser::saveAll()
{
  ...
  // GAF - 24 Aug 2010 - separate run and subrun number 
  /*snprintf(filename,"%s/GMpages_run%6.6d.ps",fFileSaveInfo.fIniDir, 
          fRunNumber);*/
  snprintf(filename,160, "%s/GMpages_run%4.4d_%4.4d.ps",fFileSaveInfo.fIniDir, 
          fRun,fSubRun);
...
string GMBrowser::makeName(const char *dir, int page)
{
  ...
  char file[256];
  snprintf(file,256, "%s/%s%2.2d_%s", dir, fTitle.c_str(), page, name.Data());
